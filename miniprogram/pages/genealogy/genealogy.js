...
Page({
  ...
  loadFilters: async function (fcampus) {
    ...
    var status_item = {
      ...
      active: false // 默认不激活，除非是第一个过滤器
    };
    filters.push(status_item);

    // 默认把第一个先激活了
    if (filters.length > 0 && !filters.some(f => f.active)) {
  ...
  fGet: async function () {
    ...
    for (const mainF of filters) {
      ...
      // 状态过滤器
      if (key === 'status') {
        if (mainF.category[0].all_active) continue; // 如果全部状态被激活则跳过

        let adoptConditions = [];
        let sterilizedValue = undefined;
        let missingValue = undefined;
        let toStarValue = undefined;

        // 遍历子类别 索引 > 0
        for (let i = 1; i < mainF.category.length; i++) {
          const subCategory = mainF.category[i];
          let subCategorySelectedValues = [];

          for (const item of subCategory.items) {
            if (subCategory.all_active || item.active) {
              subCategorySelectedValues.push(item.value);
            }
          }

          if (subCategorySelectedValues.length > 0) {
            switch (subCategory.name) {
              case '领养状态':
                adoptConditions.push(...subCategorySelectedValues);
                break;
              case '绝育状态':
                 // 最后一个有效选项生效
                 if (subCategorySelectedValues.includes(true) && subCategorySelectedValues.includes(false)) {
                     sterilizedValue = undefined; // 忽略此过滤器？或设置默认值？
                 } else if (subCategorySelectedValues.includes(true)) {
                    sterilizedValue = true;
                 } else if (subCategorySelectedValues.includes(false)) {
                    sterilizedValue = false;
                 }
                break;
              case '其他':
                 let hasMissingTrue = false;
                 let hasMissingFalse = false;
                 for(const val of subCategorySelectedValues) {
                    if (val.field === 'missing') {
                        if (val.condition === true) hasMissingTrue = true;
                        if (val.condition === false) hasMissingFalse = true;
                    }
                    if (val.field === 'to_star' && val.condition === true) {
                       toStarValue = true;
                    }
                 }
                 // missing
                 if (hasMissingTrue && hasMissingFalse) {
                     missingValue = undefined;
                 } else if (hasMissingTrue) {
                     missingValue = true;
                 } else if (hasMissingFalse) {
                     missingValue = false;
                 }
                break;
            }
        ...
        // 构建状态的查询条件
        if (adoptConditions.length > 0) {
          let adoptQueries = [];
          if (adoptConditions.includes(null)) adoptQueries.push(_.exists(false));
          const numericAdopt = adoptConditions.filter(v => v !== null);
          if (numericAdopt.length > 0) adoptQueries.push(_.in(numericAdopt));
          if (adoptQueries.length > 0) res.push({ adopt: adoptQueries.length === 1 ? adoptQueries[0] : _.or(adoptQueries) });
        }
        if (sterilizedValue !== undefined) {
          res.push({ sterilized: sterilizedValue === false ? _.or(_.eq(false), _.exists(false)) : true });
        }
        if (missingValue !== undefined) {
          res.push({ missing: missingValue === false ? _.or(_.eq(false), _.exists(false)) : true });
        }
        if (toStarValue === true) {
          res.push({ to_star: true });
        }
        continue; // 完成状态处理，移至下一个主过滤器
      }

      // 处理其他过滤器 (area, colour)
      if (key !== 'status') { // 处理非 status 过滤器
         ...
         for (const category of mainF.category) {
           ...
           for (const item of category.items) {
              // area/colour：category.all_active 应用于像特定校区这样的子分组
              if (category.all_active || item.active) {
                var choice = item.name; // 对于 area/colour
                if (item.value !== undefined) {
                   choice = item.value;
                }
                 if (item.value === null) {
                    choice = _.exists(false);
                 }
                selected.push(choice);

                if (cateFilter && !cateKeyPushed && category.name) { // 确保分类名称存在
         ...
         if(selected.length > 0) {
             let hasNull = selected.some(val => typeof val === 'object' && val !== null && val._internalType === 'db.command.exists');
             let nonNullSelected = selected.filter(val => !(typeof val === 'object' && val !== null && val._internalType === 'db.command.exists'));

             let condition = {};
             let fieldCondition = [];
             if (nonNullSelected.length > 0) fieldCondition.push(_.in(nonNullSelected));
             if (hasNull) fieldCondition.push(_.exists(false));

             if (fieldCondition.length > 1) condition[key] = _.or(fieldCondition);
             else if (fieldCondition.length === 1) condition[key] = fieldCondition[0];
             else if (hasNull) condition[key] = _.exists(false);
             else continue; // 如果此键没有任何选中项则跳过

             res.push(condition);
             if (cateFilter && cateSelected.length > 0) res.push({ [cateKey]: _.in(cateSelected) });
         }
      }
    }
